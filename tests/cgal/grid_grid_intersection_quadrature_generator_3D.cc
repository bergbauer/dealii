...
test(unsigned int refinment_domain, unsigned int refinment_boundary)
{
        ...
        const FE_Q<3, 3> finite_element(1); //or put FE::nothing
        ...
        names_and_args = {{"hyper_cube", "0.0 : 1.0 : false"}, //extreme case grid borders match complete
                        ...
                        {"hyper_rectangle", "0.0, -0.1 , 0.0: 1.0 , 0.5 ,1.0 : false"}, //difficult boundary case
                        {"hyper_rectangle", "-0.1, -0.1, 0.0 : 0.5 , 1.1, 1.0 : false"}, //only partially in domain
                        ...
                        {"hyper_ball_balanced", "-0.1 ,1.0 ,0.0 : 0.5"}}; //only partially in domain
        ...
        for (const auto &info_pair : names_and_args)
        {
                ...
                for(const auto &boolean_operation : boolean_operations)
                {
                        ...
                        std::vector<NonMatching::ImmersedSurfaceQuadrature<3>> vector_quadratures_surface;
                        std::vector<typename Triangulation<3, 3>::cell_iterator> vector_accessors;
                        ...
                        NonMatching::MappingInfo mapping_info(mapping,
                          UpdateFlags::update_quadrature_points | UpdateFlags::update_JxW_values);
                        
                        FEPointEvaluation<1, 3, 3, double> point_evaluation(mapping_info, finite_element);
                        ...
                        FEPointEvaluation<1, 3, 3, double> point_evaluation_surface(mapping_info_surface, finite_element);
