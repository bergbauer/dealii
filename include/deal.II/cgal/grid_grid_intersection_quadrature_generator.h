...
namespace CGALWrappers
{
    ...
    unsigned int                                    face_index)
  {
    ...
    if (CGAL::bounded_side_2(fitted_2D_mesh.begin(),
                             fitted_2D_mesh.end(),
                             p_1) == CGAL::ON_BOUNDARY &&
        CGAL::bounded_side_2(fitted_2D_mesh.begin(),
                             fitted_2D_mesh.end(),
                             p_2) == CGAL::ON_BOUNDARY)
      {
        ...
        for (const auto &edge_cut : polygon_out_vec[i].outer_boundary().edges())
          {
            ...
            bool on_boundary_p_1 =
              CGAL::bounded_side_2(fitted_2D_mesh.begin(),
                                   fitted_2D_mesh.end(),
                                   p_cut_1) == CGAL::ON_BOUNDARY;
            bool on_boundary_p_2 =
              CGAL::bounded_side_2(fitted_2D_mesh.begin(),
                                   fitted_2D_mesh.end(),
                                   p_cut_2) == CGAL::ON_BOUNDARY;
            ...
            if (!(on_boundary_p_1 && on_boundary_p_2))
              {
                ...
                quadrature_weights.insert(quadrature_weights.end(),
                                          weights.begin(),
                                          weights.end());
                // dont break for loop for the case where the face is split into
